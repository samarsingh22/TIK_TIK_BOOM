...
export function upsertTrackedBatch(input, options = {}) {
  ...
  const next = {
    ...
    recallDate: String(input.recallDate ?? existing?.recallDate ?? ""),
    distributionBlocked: Boolean(input.distributionBlocked ?? existing?.distributionBlocked ?? false),
    counterfeitFlagged: Boolean(input.counterfeitFlagged ?? existing?.counterfeitFlagged ?? false),
    regulatorAction: String(input.regulatorAction ?? existing?.regulatorAction ?? ""),
    productStatus: String(input.productStatus ?? existing?.productStatus ?? "ACTIVE"),
    statusUpdatedAt: String(input.statusUpdatedAt ?? existing?.statusUpdatedAt ?? nowIso),
